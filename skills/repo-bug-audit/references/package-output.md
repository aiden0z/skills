...
│   ├── bug-audit-report.html           # optional, self-contained interactive report
│   ├── findings/
│   │   ├── P1/
│   │   ├── P2/
│   │   ├── P3/
...
- Optional link to `bug-audit-report.html` when the interactive report is included.
...
`bug-audit-report.html` is optional, but recommended for final handoff packages, multi-repo audits, large Bug sets, or any result the user calls a report / delivery package / dashboard.
...
- Record `included | omitted-by-user | omitted-as-lightweight-scan | omitted-after-failure` in `quality/submission-scope.md` when the report decision affects the final package shape.
...
- When `bug-audit-report.html` is expected, run `validate_bug_package.py --require-html-report` too.
