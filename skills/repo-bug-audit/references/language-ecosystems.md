...
Each language ecosystem has different default behaviors around numeric types, time APIs, and serialization — behaviors you already know. When examining code, keep these language-specific gotchas in mind:

- **Numeric precision**: which types silently overflow or lose precision on division/conversion in this language?
- **Time and clock**: which APIs give wall-clock vs monotonic time? Which are thread-safe?
- **Serialization**: which parsers allow code execution by default? Which silently drop unknown fields or coerce types?

Apply your existing language knowledge. Cite specific call sites when writing evidence.
