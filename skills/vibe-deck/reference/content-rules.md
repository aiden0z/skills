...
Slides use `lucide-react` for all icons. Emoji characters (📧📊✅❌ etc.) render inconsistently across platforms, can't be styled (no color/size control), and look unprofessional in presentations.

**Pattern for data-driven icons:**
```jsx
...
Store the component reference (not a string) in data arrays, then render with `<item.icon size={N} />`. This allows sizing and color control via props and className.

**Status icons** follow the semantic color system:
- `<CheckCircle size={13} className="text-green-500" />` — success/positive
- `<AlertTriangle size={13} className="text-neutral-400" />` — warning/concern
- `<XCircle size={13} className="text-red-400" />` — failure/negative

The `lucide-react` package is included in the template's `package.json` — no extra install needed.
